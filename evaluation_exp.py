...
def evaluate_in_detail(predictions, gold, tag2idx):
    correct_prediction = (predictions==gold)
    for tag, idx in tag2idx.items():
        if tag=='O' or tag=='0':
            continue
        tag_gold = (gold==idx)
        correct = (correct_prediction & tag_gold).long().sum().item()
        tot = tag_gold.long().sum().item()
        if tot==0:
            ratio = 'nan'
        else: 
            ratio = correct / tot
        print(f"{tag}: {correct} / {tot} ({ratio})")




def test_in_detail(model: Transformer, data: DataManager, batch_size: int, savepath: str, devices: list):
    ...
    evaluate_in_detail(test_predictions, test_tags, data.tag2idx)
...
def main():
    ...
    test_in_detail(model, data, args.batch_size, args.savepath, devices)
